...
def select_model():
    model = st.sidebar.radio(
        "今回使用するモデルは:", ("GPT-3.5", "GPT-3.5-16k", "GPT-4"))
    if model == "GPT-3.5":
    ...
    st.session_state.overlap = st.sidebar.slider(
        "チャンク間の重複文字数:", min_value=100, max_value=0, value=0, step=10)
...
def document_summarize(llm, docs):
    ...
    with get_openai_callback() as cb:
        ...
        response = chain({
            ...
            "operlap": st.session_state.overlap  # 長いコンテキストを扱えるモデルを利用するようにするために必要
        },
