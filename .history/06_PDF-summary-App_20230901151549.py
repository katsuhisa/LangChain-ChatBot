...
def load_qdrant():  # ベクトルDBを操作するクライアントを準備・設定
    client = QdrantClient(path=urllib.parse.quote(QDRANT_PATH))
...
def build_vector_store(pdf_text):  # PDFのテキストをEmbeddingにしてベクトルDBに保存
    ...
    qdrant.from_texts(
        pdf_text,
        OpenAIEmbeddings(),
        url=urllib.parse.quote(QDRANT_PATH),
        # api_key="<qdrant-api-key>",
        collection_name=COLLECTION_NAME
    )


def page_pdf_upload_and_build_vector_db():  # PDFのアップロード〜ベクトりDB構築
...
def main():
    ...
    selection = st.sidebar.radio("ページ", ["PDFアップロード用ページ", "Ask My PDF(s)"])
    ...
    # qdrant_clientパッケージのバージョンを取得
    qdrant_client_version = pkg_resources.get_distribution(
        "qdrant_client").version
    print("qdrant_client version:", qdrant_client_version)
